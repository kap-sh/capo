...
from capo_agent_registry._auth._identity import (
    Credentials,
    Identity,
)
from capo_agent_registry._services._aws_config import (
    _load_profile,
    active_profile,
    config_file,
)
...
# they need the `sso` extra: `pip install capo-agent-registry[sso]`. Without it the imports
...
def require_dependency(module: object | None, package: str) -> None:
    if module is None:
        raise MissingDependencyError(
            ...
            "installed; reinstall with the sso feature enabled: capo-agent-registry[sso]"
        )
