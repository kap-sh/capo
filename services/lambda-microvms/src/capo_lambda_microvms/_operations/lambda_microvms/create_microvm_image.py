"""Generated from Smithy shape ``com.amazonaws.lambdamicrovms#CreateMicrovmImage``."""
...
def handle_response(
    ...
    out: capo_lambda_microvms.types.create_microvm_image_response.CreateMicrovmImageResponse = capo_lambda_microvms.types.create_microvm_image_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2025-09-09/microvm-images"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lambda_microvms.types.create_microvm_image_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_microvm_image(
