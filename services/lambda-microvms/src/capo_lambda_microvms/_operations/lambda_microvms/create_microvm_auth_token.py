"""Generated from Smithy shape ``com.amazonaws.lambdamicrovms#CreateMicrovmAuthToken``."""
...
def handle_response(
    ...
    out: capo_lambda_microvms.types.create_microvm_auth_token_response.CreateMicrovmAuthTokenResponse = capo_lambda_microvms.types.create_microvm_auth_token_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/2025-09-09/microvms/{microvmIdentifier}/auth-token"
    )
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_lambda_microvms.types.create_microvm_auth_token_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_microvm_auth_token(
