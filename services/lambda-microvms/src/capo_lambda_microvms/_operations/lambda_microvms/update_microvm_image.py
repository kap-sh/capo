"""Generated from Smithy shape ``com.amazonaws.lambdamicrovms#UpdateMicrovmImage``."""
...
def handle_response(
    ...
    out: capo_lambda_microvms.types.update_microvm_image_response.UpdateMicrovmImageResponse = capo_lambda_microvms.types.update_microvm_image_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2025-09-09/microvm-images/{imageIdentifier}"
    url = url.replace("{imageIdentifier}", quote(input_["image_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lambda_microvms.types.update_microvm_image_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_microvm_image(
