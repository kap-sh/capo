"""Generated from Smithy shape ``com.amazonaws.lambdamicrovms#MicrovmImageSummary``."""
...
class MicrovmImageSummary(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: MicrovmImageSummary) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> MicrovmImageSummary:
    out: MicrovmImageSummary = {}  # type: ignore[typeddict-item]
    if data.get("imageArn") is not None:
        out["image_arn"] = data["imageArn"]
    else:
        raise DeserializationError("MicrovmImageSummary.image_arn required")
    if data.get("name") is not None:
        out["name"] = data["name"]
    else:
        raise DeserializationError("MicrovmImageSummary.name required")
    if data.get("state") is not None:
        ...
        raise DeserializationError("MicrovmImageSummary.state required")
    ...
    if data.get("createdAt") is not None:
        ...
        raise DeserializationError("MicrovmImageSummary.created_at required")
    return out
