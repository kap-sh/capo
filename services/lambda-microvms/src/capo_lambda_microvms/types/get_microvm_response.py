"""Generated from Smithy shape ``com.amazonaws.lambdamicrovms#GetMicrovmResponse``."""
...
class GetMicrovmResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: GetMicrovmResponse) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> GetMicrovmResponse:
    out: GetMicrovmResponse = {}  # type: ignore[typeddict-item]
    if data.get("microvmId") is not None:
        out["microvm_id"] = data["microvmId"]
    else:
        raise DeserializationError("GetMicrovmResponse.microvm_id required")
    if data.get("state") is not None:
        ...
        raise DeserializationError("GetMicrovmResponse.state required")
    if data.get("endpoint") is not None:
        out["endpoint"] = data["endpoint"]
    else:
        raise DeserializationError("GetMicrovmResponse.endpoint required")
    if data.get("imageArn") is not None:
        out["image_arn"] = data["imageArn"]
    else:
        raise DeserializationError("GetMicrovmResponse.image_arn required")
    if data.get("imageVersion") is not None:
        out["image_version"] = data["imageVersion"]
    else:
        raise DeserializationError("GetMicrovmResponse.image_version required")
    ...
    if data.get("maximumDurationInSeconds") is not None:
        ...
        raise DeserializationError(
            "GetMicrovmResponse.maximum_duration_in_seconds required"
        )
    if data.get("startedAt") is not None:
        ...
        raise DeserializationError("GetMicrovmResponse.started_at required")
