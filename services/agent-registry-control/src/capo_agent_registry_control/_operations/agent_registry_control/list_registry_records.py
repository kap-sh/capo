"""Generated from Smithy shape ``com.amazonaws.agentregistrycontrol#ListRegistryRecords``."""
...
def handle_response(
    ...
    out: capo_agent_registry_control.types.list_registry_records_response.ListRegistryRecordsResponse = capo_agent_registry_control.types.list_registry_records_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/registries/{registryId}/records-list"
    url = url.replace("{registryId}", quote(input_["registry_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_agent_registry_control.types.list_registry_records_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
