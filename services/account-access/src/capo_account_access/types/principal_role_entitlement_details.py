"""Generated from Smithy shape ``com.amazonaws.accountaccess#PrincipalRoleEntitlementDetails``."""
...
class PrincipalRoleEntitlementDetails(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: PrincipalRoleEntitlementDetails) -> dict:
    out: dict = {}
    import capo_account_access.types.principal
...
def deserialize_json(data: dict) -> PrincipalRoleEntitlementDetails:
    out: PrincipalRoleEntitlementDetails = {}  # type: ignore[typeddict-item]
    if data.get("principal") is not None:
        ...
        raise DeserializationError("PrincipalRoleEntitlementDetails.principal required")
    if data.get("roleArn") is not None:
        out["role_arn"] = data["roleArn"]
    else:
        raise DeserializationError("PrincipalRoleEntitlementDetails.role_arn required")
    if data.get("account") is not None:
        out["account"] = data["account"]
    else:
        raise DeserializationError("PrincipalRoleEntitlementDetails.account required")
