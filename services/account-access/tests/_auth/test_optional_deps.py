...
import pytest
from capo_account_access._auth import _providers
...
def test_sso_configured_reports_missing_extra(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        MissingDependencyError, match=r"capo-account-access\[sso\]"
    ) as excinfo:
...
def test_sso_configured_reports_missing_extra_async(tmp_path: Path) -> None:
    _write_config(tmp_path, _SSO_PROFILE)
    with pytest.raises(MissingDependencyError, match=r"capo-account-access\[sso\]"):
