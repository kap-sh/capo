"""Generated from Smithy shape ``com.amazonaws.pricingplanmanager#CancelSubscription``."""
...
def handle_response(
    ...
    out: capo_pricing_plan_manager.types.cancel_subscription_output.CancelSubscriptionOutput = {
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/CancelSubscription"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "if_match" in input_:
        headers["If-Match"] = input_["if_match"]
    body: bytes | None = json.dumps(
        capo_pricing_plan_manager.types.cancel_subscription_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
