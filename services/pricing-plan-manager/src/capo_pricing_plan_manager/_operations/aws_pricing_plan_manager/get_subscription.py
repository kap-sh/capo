"""Generated from Smithy shape ``com.amazonaws.pricingplanmanager#GetSubscription``."""
...
def handle_response(
    ...
    out: capo_pricing_plan_manager.types.get_subscription_output.GetSubscriptionOutput = {
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/GetSubscription"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_pricing_plan_manager.types.get_subscription_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
