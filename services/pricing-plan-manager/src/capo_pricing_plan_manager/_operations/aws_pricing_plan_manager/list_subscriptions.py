"""Generated from Smithy shape ``com.amazonaws.pricingplanmanager#ListSubscriptions``."""
...
def handle_response(
    ...
    out: capo_pricing_plan_manager.types.list_subscriptions_output.ListSubscriptionsOutput = capo_pricing_plan_manager.types.list_subscriptions_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/ListSubscriptions"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_pricing_plan_manager.types.list_subscriptions_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
