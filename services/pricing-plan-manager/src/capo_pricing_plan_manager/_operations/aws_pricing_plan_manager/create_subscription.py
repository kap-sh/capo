"""Generated from Smithy shape ``com.amazonaws.pricingplanmanager#CreateSubscription``."""
...
def handle_response(
    ...
    out: capo_pricing_plan_manager.types.create_subscription_output.CreateSubscriptionOutput = {
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/CreateSubscription"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_pricing_plan_manager.types.create_subscription_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
