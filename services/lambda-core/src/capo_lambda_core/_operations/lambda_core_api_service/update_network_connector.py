"""Generated from Smithy shape ``com.amazonaws.lambdacore#UpdateNetworkConnector``."""
...
import capo_lambda_core.errors.resource_not_found_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceNotFoundException":
            raise capo_lambda_core.errors.resource_not_found_exception.ResourceNotFoundException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_lambda_core.types.update_network_connector_response.UpdateNetworkConnectorResponse = capo_lambda_core.types.update_network_connector_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2026-04-04/network-connectors/{Identifier}"
    url = url.replace("{Identifier}", quote(input_["identifier"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lambda_core.types.update_network_connector_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_network_connector(
