...
from capo_network_security_manager._auth._identity import (
    Credentials,
    Identity,
)
from capo_network_security_manager._services._aws_config import (
    _load_profile,
    active_profile,
    config_file,
)
...
# they need the `sso` extra: `pip install capo-network-security-manager[sso]`. Without it the imports
...
def require_dependency(module: object | None, package: str) -> None:
    if module is None:
        raise MissingDependencyError(
            ...
            "installed; reinstall with the sso feature enabled: capo-network-security-manager[sso]"
        )
