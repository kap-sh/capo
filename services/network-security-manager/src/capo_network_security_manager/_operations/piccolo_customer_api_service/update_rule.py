"""Generated from Smithy shape ``com.amazonaws.networksecuritymanager#UpdateRule``."""
...
def handle_response(
    ...
    out: capo_network_security_manager.types.update_rule_output.UpdateRuleOutput = (
        capo_network_security_manager.types.update_rule_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/rules/{ruleIdentifier}"
    url = url.replace("{ruleIdentifier}", quote(input_["rule_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_network_security_manager.types.update_rule_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
