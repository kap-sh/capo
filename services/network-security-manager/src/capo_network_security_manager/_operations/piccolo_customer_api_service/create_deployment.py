"""Generated from Smithy shape ``com.amazonaws.networksecuritymanager#CreateDeployment``."""
...
import capo_network_security_manager.errors.service_quota_exceeded_exception
import capo_network_security_manager.errors.service_unavailable_exception
import capo_network_security_manager.errors.tag_policy_violation_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ServiceQuotaExceededException":
            raise capo_network_security_manager.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data, message
            )
        case "ServiceUnavailableException":
            raise capo_network_security_manager.errors.service_unavailable_exception.ServiceUnavailableException.from_json(
                data, message
            )
        case "TagPolicyViolationException":
            raise capo_network_security_manager.errors.tag_policy_violation_exception.TagPolicyViolationException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_network_security_manager.types.create_deployment_output.CreateDeploymentOutput = capo_network_security_manager.types.create_deployment_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/deployments"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_network_security_manager.types.create_deployment_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
