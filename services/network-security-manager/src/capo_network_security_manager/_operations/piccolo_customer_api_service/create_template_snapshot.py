"""Generated from Smithy shape ``com.amazonaws.networksecuritymanager#CreateTemplateSnapshot``."""
...
def handle_response(
    ...
    out: capo_network_security_manager.types.create_template_snapshot_output.CreateTemplateSnapshotOutput = capo_network_security_manager.types.create_template_snapshot_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/templates/{templateIdentifier}/snapshots"
    url = url.replace(
        "{templateIdentifier}", quote(input_["template_identifier"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_network_security_manager.types.create_template_snapshot_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_template_snapshot(
