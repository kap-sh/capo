"""Generated from Smithy shape ``com.amazonaws.networksecuritymanager#UpdatePolicy``."""
...
def handle_response(
    ...
    out: capo_network_security_manager.types.update_policy_output.UpdatePolicyOutput = (
        capo_network_security_manager.types.update_policy_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/policies/{policyIdentifier}"
    url = url.replace("{policyIdentifier}", quote(input_["policy_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_network_security_manager.types.update_policy_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
