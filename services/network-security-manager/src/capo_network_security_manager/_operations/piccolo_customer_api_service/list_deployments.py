"""Generated from Smithy shape ``com.amazonaws.networksecuritymanager#ListDeployments``."""
...
def handle_response(
    ...
    out: capo_network_security_manager.types.list_deployments_output.ListDeploymentsOutput = capo_network_security_manager.types.list_deployments_output.deserialize_json(
...
def build_request(
    ...
    if "status" in input_:
        params.append(
            (
                "status",
                capo_network_security_manager.types.entity_status_filter.serialize_json(
                    input_["status"]
                ),
            )
        )
