"""Generated from Smithy shape ``com.amazonaws.networksecuritymanager#CreateScopeSnapshot``."""
...
def handle_response(
    ...
    out: capo_network_security_manager.types.create_scope_snapshot_output.CreateScopeSnapshotOutput = capo_network_security_manager.types.create_scope_snapshot_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/scopes/{scopeIdentifier}/snapshots"
    url = url.replace("{scopeIdentifier}", quote(input_["scope_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_network_security_manager.types.create_scope_snapshot_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_scope_snapshot(
