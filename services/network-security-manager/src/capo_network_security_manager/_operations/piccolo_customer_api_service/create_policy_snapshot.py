"""Generated from Smithy shape ``com.amazonaws.networksecuritymanager#CreatePolicySnapshot``."""
...
def handle_response(
    ...
    out: capo_network_security_manager.types.create_policy_snapshot_output.CreatePolicySnapshotOutput = capo_network_security_manager.types.create_policy_snapshot_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/policies/{policyIdentifier}/snapshots"
    url = url.replace("{policyIdentifier}", quote(input_["policy_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_network_security_manager.types.create_policy_snapshot_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_policy_snapshot(
