"""Generated from Smithy shape ``com.amazonaws.networksecuritymanager#ListRuleSnapshotsOutput``."""
...
class ListRuleSnapshotsOutput(TypedDict, closed=True):
    ...
    snapshots: "capo_network_security_manager.types.rule_summary_list.RuleSummaryList"
    """<p>The snapshots of the rule.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ListRuleSnapshotsOutput) -> dict:
    out: dict = {}
    ...
    out["snapshots"] = (
        capo_network_security_manager.types.rule_summary_list.serialize_json(
            value["snapshots"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> ListRuleSnapshotsOutput:
    out: ListRuleSnapshotsOutput = {}  # type: ignore[typeddict-item]
    ...
    if data.get("snapshots") is not None:
        ...
        raise DeserializationError("ListRuleSnapshotsOutput.snapshots required")
    return out
