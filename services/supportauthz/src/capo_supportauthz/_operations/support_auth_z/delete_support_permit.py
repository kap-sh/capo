"""Generated from Smithy shape ``com.amazonaws.supportauthz#DeleteSupportPermit``."""
...
def handle_response(
    ...
    out: capo_supportauthz.types.delete_support_permit_output.DeleteSupportPermitOutput = capo_supportauthz.types.delete_support_permit_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/support-permits/{supportPermitIdentifier}"
    url = url.replace(
        "{supportPermitIdentifier}", quote(input_["support_permit_identifier"], safe="")
    )
...
async def async_delete_support_permit(
